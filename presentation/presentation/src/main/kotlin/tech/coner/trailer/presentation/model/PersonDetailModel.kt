...
class PersonDetailModel(
    ...
    fun setId(id: UUID) {
        mutatePendingItem { it.copy(id = id) }
    }

    val firstName
        get() = adapter.firstName(pendingItem)
    fun setFirstName(firstName: String) {
        mutatePendingItem { it.copy(firstName = firstName) }
    }

    val lastName
        get() = adapter.lastName(pendingItem)
    fun setLastName(lastName: String) {
        mutatePendingItem { it.copy(lastName = lastName) }
    }

    val clubMemberId
        get() = adapter.clubMemberId(pendingItem)
    fun setClubMemberId(clubMemberId: String) {
        mutatePendingItem { it.copy(clubMemberId = clubMemberId) }
    }

    val motorsportRegId
        get() = adapter.motorsportRegMemberId(pendingItem)
    fun setMotorsportRegId(motorsportRegId: String) {
        mutatePendingItem { it.copy(motorsportReg = Person.MotorsportRegMetadata(motorsportRegId)) }
    }
}