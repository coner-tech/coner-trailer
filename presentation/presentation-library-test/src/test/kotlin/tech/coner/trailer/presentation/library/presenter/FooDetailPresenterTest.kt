...
import tech.coner.trailer.presentation.library.model.original
...
class FooDetailPresenterTest {
    ...
    @Test
    fun whenItsModelNameChangedItsItemModelShouldBeValid() = runTest {
        val id = Foo.Id(FOO_ID_FOO)
        val presenter = createPresenter(id)
        launch { presenter.load() }
        val model = presenter.awaitLoadedModel() as LoadableModel.Loaded
        val newName = "bax"
